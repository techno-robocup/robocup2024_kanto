...
while True:
    ...
    if isblack(BOTTOM_LEFT_OBJ.h, BOTTOM_LEFT_OBJ.s, BOTTOM_LEFT_OBJ.v):
        ...
        while isgreen(BOTTOM_LEFT_OBJ.h,BOTTOM_LEFT_OBJ.s,BOTTOM_LEFT_OBJ.v):
            updatedata()
            MOTORL.run(DEFAULTTURNSPEED)
            MOTORR.run(DEFAULTTURNSPEED)
        ...
        while isgreen(BOTTOM_RIGHT_OBJ.h,BOTTOM_RIGHT_OBJ.s,BOTTOM_RIGHT_OBJ.v):
            updatedata()
            MOTORL.run(DEFAULTTURNSPEED)
            MOTORR.run(DEFAULTTURNSPEED)
        ...
        while True:
            updatedata()
            if TOUCHR.pressed():
                MOTORL.run(DEFAULTTURNSPEED-80)
                MOTORR.run(DEFAULTTURNSPEED)
                time.sleep(0.3)
            else:
                MOTORL.run(DEFAULTTURNSPEED)
                MOTORR.run(DEFAULTTURNSPEED-80)
            if isblack(BOTTOM_LEFT_OBJ.h,BOTTOM_LEFT_OBJ.s,BOTTOM_LEFT_OBJ.v):
                WASLBLACK = True
            if isblack(BOTTOM_RIGHT_OBJ.h,BOTTOM_RIGHT_OBJ.s,BOTTOM_RIGHT_OBJ.v):
                WASRBLACK = True
